...
func QueryPrometheus(params PrometheusQueryParams) (string, error) {
	if params.Config != nil && params.PrometheusmNamespace != "" && params.PrometheusServiceName != "" {
		restConfig := params.Config.Client().RESTConfig()
		restConfig.GroupVersion = &schema.GroupVersion{Group: "", Version: "v1"}
		restConfig.NegotiatedSerializer = scheme.Codecs.WithoutConversion()
		restClient, err := rest.RESTClientFor(restConfig)
		if err != nil {
			return "", fmt.Errorf("error when getting rest client: %v", err)
		}
